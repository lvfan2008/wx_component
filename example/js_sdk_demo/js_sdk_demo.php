<?php
/**
 * 采用微信官方的js_sdk_demo源码，实现PHP后端代码
...
$notifyUrl = $baseUrl . "/js_sdk_wx_pay_notify.php";

$jsSdkDemo = new JsSdkDemo($wxComponentService, $appId, $wxPay);
$wxCfgJsSign = $jsSdkDemo->getWxConfigJsSignPackage($url);
$wxPaySign = $jsSdkDemo->getWxPaySign($notifyUrl);
$wxAddCardInfo = $jsSdkDemo->getAddCardInfo();
$wxChooseCardSign = $jsSdkDemo->getChooseCardSign();
$wxTestCardId = JsSdkDemo::TestCardId;
...
<script>
     ...
     */
        ...
        // 用于wx.config
    var g_wxCfgJsSign = <?php echo json_encode($wxCfgJsSign);?>;

    // 用于微信支付v3版本
    var g_wxPaySign = <?php echo json_encode($wxPaySign);?>;

    // 用于卡券 wx.addCard
    var g_wxAddCardExt = '<?php echo $wxAddCardInfo['cardExt'];?>';

    // 用于卡券 wx.chooseCard
    var g_wxChooseCardSign = <?php echo json_encode($wxChooseCardSign);?>;

    // 测试卡券ID
    var g_wxTestCardId = "<?php echo $wxTestCardId;?>";
...
<script>
     ...
     */
    wx.config({
        debug: true, // 正式的时候，设置为false
        appId: g_wxCfgJsSign.appId,
        timestamp: g_wxCfgJsSign.timestamp,
        nonceStr: g_wxCfgJsSign.nonceStr,
        signature: g_wxCfgJsSign.signature,
        jsApiList: [
            'checkJsApi',
            'onMenuShareTimeline',
...
<script src="js/demo.js?v=1.2.5"></script>
</html>
